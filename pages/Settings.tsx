...
import { FileText, FileSpreadsheet, Download, Check, Copy, Share2, Zap, Lock, Shield, Bell, User, Mail, Database } from 'lucide-react';
  ...
  return (
    <div className="space-y-6">
       ...
       <div className="flex flex-col">
         <h1 className="text-2xl font-bold text-gray-900">{isAdmin ? 'Admin Settings' : 'Export & Settings'}</h1>
         <p className="text-gray-500 text-sm mt-1">
            {isAdmin ? 'Manage system preferences and administrator profile.' : 'Manage your data exports and personalize your application settings.'}
         </p>
       </div>

       {/* USER ONLY: Referral Program */}
       {!isAdmin && (
       ...
       {isAdmin && (
           <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm">
                    ...
                    <div className="space-y-3">
                        <div className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                            ...
                            <span className="text-xs bg-emerald-100 text-emerald-700 px-2 py-1 rounded font-bold">Full Access</span>
                        </div>
                        <div className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                            <span className="text-sm font-medium text-gray-700">System Logs</span>
                            <span className="text-xs bg-emerald-100 text-emerald-700 px-2 py-1 rounded font-bold">View Only</span>
                        </div>
                        <div className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                            <span className="text-sm font-medium text-gray-700">Billing Controls</span>
                            <span className="text-xs bg-emerald-100 text-emerald-700 px-2 py-1 rounded font-bold">Read/Write</span>
                        </div>
                    </div>
                </div>
                ...
                <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm">
                    ...
                    <p className="text-sm text-gray-500 mb-4">Trigger system-wide events or maintenance modes.</p>
                    <button className="w-full py-2 border border-red-200 text-red-600 hover:bg-red-50 rounded-lg text-sm font-medium transition-colors mb-2">
                        Clear System Cache
                    </button>
                    <button className="w-full py-2 border border-gray-200 text-gray-600 hover:bg-gray-50 rounded-lg text-sm font-medium transition-colors">
                        View Audit Logs
                    </button>
                </div>
           </div>
       ...
       {/* Shared: User/Admin Profile Settings */}
       <div className="bg-white p-8 rounded-xl border border-gray-100 shadow-sm">
          <h2 className="text-lg font-bold text-gray-900 mb-1">
              {isAdmin ? 'Administrator Profile' : 'User Settings'}
          </h2>
          <p className="text-sm text-gray-500 mb-8">
              {isAdmin ? 'Update your admin contact details and security notifications.' : 'Update your profile, notification preferences, and GSTIN status.'}
          </p>
              ...
              <div className="space-y-4">
                  ...
                  <div>
                      ...
                      <div className="flex items-center px-3 py-2 border border-gray-200 rounded-lg bg-gray-50">
                          ...
                          <span className="text-sm text-gray-900">{user?.email || 'admin@pinnacle.com'}</span>
                      </div>
                  </div>
              </div>
